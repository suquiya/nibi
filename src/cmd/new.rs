...
pub fn threed_cmd() -> Command {
	Command::with_all_field(
		...
		"nibi new [site(project)/ingot(post)/igata(template)/igata_set(theme)]".to_owned(),
		flags![],
		vector![],
		alias!["n", "create"],
		String::default(),
		vector![sub_help(), new_igt_pack_cmd(), new_recipe_cmd()],
	)
}
...
pub fn new_igt_pack_cmd() -> Command {
	Command::with_all_field(
		...
		String::default(),
		license![],
		Some("create new igata_pack (, or theme).".to_owned()),
